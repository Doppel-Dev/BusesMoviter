# Buses Moviter 🚌

Sitio web corporativo y sistema de cotización para **Buses Moviter**, empresa líder en transporte privado de personal y viajes especiales en Chile.

## Características

- **Diseño:** Interfaz moderna, responsiva y corporativa construida con React y Bootstrap.
- **MoviBot:** Asistente virtual inteligente que guía a los usuarios a través del proceso de cotización.
- **Formulario de Cotización:** Sistema integrado para capturar requerimientos detallados de transporte.
- **Galería de Servicios:** Visualización de la flota y servicios ofrecidos.
- **Optimización de Rendimiento:** Uso de Framer Motion para animaciones fluidas y mejores prácticas de React.

## Stack Tecnológico

### Frontend
- **React 18** (Vite)
- **Bootstrap / React-Bootstrap** (Estilizado)
- **Framer Motion** (Animaciones)
- **React Icons**

### Backend
- **Node.js**
- **Express**
- **CORS**

## Estructura del Proyecto

```text
├── buses-moviter-frontend/   # Aplicación React (Interfaz de usuario)
├── buses-moviter-backend/    # Servidor Node.js (Procesamiento de datos)
├── logo/                     # Recursos de identidad corporativa
└── imagenes/                 # Galería y contenido visual
```

## Instalación y Uso

### Requisitos
- Node.js (v16 o superior)
- npm o yarn

### Configuración del Frontend
1. Entrar a la carpeta: `cd buses-moviter-frontend`
2. Instalar dependencias: `npm install`
3. Iniciar modo desarrollo: `npm run dev`

### Configuración del Backend
1. Entrar a la carpeta: `cd buses-moviter-backend`
2. Instalar dependencias: `npm install`
3. Iniciar servidor: `npm start`

---
Desarrollado para Buses Moviter.
